...
;;
;; Modified by Matthew L Fidler to 
;;  * take care of unbalanced || in comments.
;;  * take care of a specified box length.
;;  * rebox without wrapping.
;;  * uncomment a rebox.
;;  * Changed rebox to return 't if inside a box and nil if not instead of throwing an error.
;;
...
;;
;; With rebox-set-box-size, the margin is at least the fill-column length. (MLF)
;;

(defvar rebox-set-box-size nil
  "*If not nil, then the box size is fixed, not variable.")

(defvar rebox-no-wrap nil
  "*If not nil, then the box is wrapped.  Otherwise the comment stuff is removed, and then the box is wrapped.")

(defvar rebox-uncomment nil
  "* When not till, the box is uncommented.")
...
(defun rebox-toggle-wrap ()
  "* Rebox will toggle the automatic wrapping within the comment."
  (interactive)
  (if rebox-no-wrap
      (progn
	(message "Rebox will word-wrap.")
	(setq rebox-no-wrap nil)
	)
    (message "Rebox no longer will wrap.")
    (setq rebox-no-wrap 't)
    )
)

(defun rebox-toggle-uncomment ()
  "* Rebox will toggle whether the region is uncommented, or commented."
  (interactive)
  (if rebox-uncomment
      (progn
	(setq rebox-uncomment nil)
	(message "Rebox will now comment out regions.")
	)
    (setq rebox-uncomment 't)
    (message "Rebox will uncomment regions.")
    )
  )
...
;;
;; Modified by MLF to have a default margin length...
;;
  (let ((margin 0) period)
    (goto-char (point-min))
    (while (not (eobp))
	...
	(forward-char 1))
      ...
      (forward-char 1))
    (if rebox-set-box-size
	(setq margin (max margin fill-column)))
    margin))
...
(defun rebox-find-and-narrow ()
  "Changed to return true if sucessful, and nil if unsucessful."
  (setq ret 't)
  (save-excursion
    (beginning-of-line)
    (if (looking-at "^ *[`\+\\]*--+ *\n")
...
(defun rebox-reconstruct (refill margin language quality type)
  ...
  (let (right-margin nw nn ne ww ee sw ss se x xx)
	...
	(progn
	  (goto-char (point-min))
	  (setq tmp "^ \\{")
	  (setq tmp (concat tmp (number-to-string (length ww))))
	  (setq tmp (concat tmp "\\}"))
	...
	(while (re-search-forward (rebox-regexp-end language) nil t)
	  ...
	  (progn
	    ...
	    (while (re-search-forward "^\\( *\\)|\\(.*\\)| *$" nil t)
	      ...
	      (replace-match "\\1 \\2" t))
	    ;;
	    ;; Also take out lines that are not balanced (therefore you can just start 
	    ;; typing somewhere and it will work. Modified MLF 2006
	    ;;
	    (goto-char (point-min))
	    (let (start end mar addLine)
	      (while (re-search-forward "^\\( *\\)|\\( *\\)\\(.*\\)$" nil t)
		(setq quality REBOX_QUALITY_ROUNDED_TWO)
		(setq addLine 0)
		;; Make sure the space before box is intact.
		(if (< (length (match-string 2)) 1)
		    (progn
		      (replace-match "\\1  \\3" t)
		      (setq mar (concat "\n  " (match-string 1) ))
		      )
		  (replace-match "\\1 \\2\\3")
		  (setq mar (concat (concat "\n " (match-string 2)) (match-string 1) ))
		  )
		...
		(if (not (re-search-forward "^\\([^|]*\\)| *$" nil t))
		  ...
		  (while (re-search-forward "^\\(.*\\)$" end t)
		    (replace-match (concat mar "\\1") t)
		    (forward-char 1)
		    (setq start (point))
		    (setq addLine (+ addLine 1))
		    ;;
		    ;; Add a line
		    ;;
		    (goto-char end)
	    ...
	    ;; Now take of the `------------- that seems to haunt the life of this script...
	    (goto-char (point-min))
	    (if (= quality REBOX_QUALITY_ROUNDED_TWO)
		(progn
		  (while (re-search-forward "\\(`-*\\)" nil t)
		    (replace-match "" t))
		  )
